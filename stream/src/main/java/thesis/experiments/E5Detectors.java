...
public class E5Detectors {
    ...
    public static final class RunResult {
        ...
        public double accuracy, kappa, kappaPer, recoveryTime, ramHours, featureStability;
    ...
    public static void main(String[] args) throws Exception {
        String configPath = args.length > 0 ? args[0]
                : "stream/configs/e5_detectors.json";
                    ...
                    for (int seed : cfg.seeds) {
                        for (String model : cfg.models) {
                            for (String det : cfg.detectors) {
                                try {
                                    RunResult r = runOne(cfg, g, m, det, model, seed, w);
                                    writeAlarmsCsv(a, r);
                                    all.add(r);
                                } catch (Exception ex) {
                                    System.err.printf("[E5][FAIL] %s|%s|%s|%s|seed=%d -> %s%n",
                                            g.name, det, model, mag, seed, ex);
    ...
    public RunResult runOne(Cfg cfg, GeneratorSpec g, Magnitude mag,
                            ...
                            PrintWriter wcsv) throws Exception {
        ...
        while (stream.hasMoreInstances() && n < cfg.maxInstances) {
            ...
            if (cfg.logEvery > 0 && n % cfg.logEvery == 0) {
                ...
                wcsv.printf(Locale.ROOT,
                        "%d,%s,%s,%s,%s,%d,%d,%d,%.4f,%.4f,%.4f,%.4f,%.4f,%d,%d,%d,%.4f,%.6f%n",
        ...
        System.out.printf(Locale.ROOT,
                "[E5] %-18s %-18s %-8s %-6s seed=%d  k=%.4f  drift=%d  TP=%d FP=%d FN=%d  P=%.3f R=%.3f F1=%.3f delay=%.1f  recov=%.1f%n",
                g.name, detName, modelName, mag.name(), seed,
        ...
        try (PrintWriter w = new PrintWriter(new FileWriter(path.toFile()))) {
            ...
            for (RunResult r : all) {
                w.printf(Locale.ROOT,
                        "%s,%s,%s,%s,%d,%d,%d,%.4f,%.4f,%.4f,%.4f,%.4f,%.6f,%d,%d,"
                                + "%d,%d,%d,%.4f,%.4f,%.4f,%.6f,%.2f,\"%s\"%n",
    ...
    public static void writeAggregated(Cfg cfg, List<RunResult> all) throws Exception {
        ...
        try (PrintWriter w = new PrintWriter(new FileWriter(p.toFile()))) {
            ...
            for (Map.Entry<String, List<RunResult>> e : grp.entrySet()) {
                ...
                w.printf(Locale.ROOT,
                        "%s,%s,%s,%s,%d,%.4f,%.4f,%.4f,%.4f,%.2f,%.4f,%.4f,%.4f,%.4f,%.6f,%.2f%n",
